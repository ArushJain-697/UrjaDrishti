    ...
    <div className="hidden items-center gap-1.5 sm:flex">
      ...
      <div className="impact-popup-anchor relative">
        ...
        {openPopup === 'carbon' && (
          <div className="absolute left-1/2 top-full z-[300] mt-2 w-72 -translate-x-1/2 rounded-xl border border-line bg-surface-bg p-4 shadow-2xl animate-fade-in">
            ...
            <h4 className="mb-3 text-sm font-semibold text-[#22c55e]">🌱 {t('carbonImpactBreakdown')}</h4>
            <div className="flex flex-col gap-2 text-xs">
              <div className="flex justify-between border-b border-line pb-1.5">
                <span className="text-muted-text">{t('totalRenewableGen')}</span>
                <span className="font-semibold text-main-text">{totalMwh.toLocaleString('en-IN')} MWh</span>
              </div>
              <div className="flex justify-between border-b border-line pb-1.5">
                <span className="text-muted-text">{t('gridEmissionFactor')}</span>
                <span className="font-semibold text-main-text">{EMISSION_FACTOR} tCO₂/MWh</span>
              </div>
              <div className="flex justify-between border-b border-line pb-1.5">
                <span className="text-muted-text">{t('carbonAvoided')}</span>
                <span className="font-bold text-[#22c55e]">{Math.round(carbon).toLocaleString('en-IN')} {lang === 'kn' ? 'ಟನ್' : 'tonnes'}</span>
              </div>
              <div className="flex justify-between pb-0.5">
                <span className="text-muted-text">{t('equivalentTo')}</span>
                <span className="font-semibold text-main-text">🌳 {Math.round(carbon * 50).toLocaleString('en-IN')} {t('treesYr')}</span>
              </div>
            </div>
            <div className="mt-3 rounded-md bg-[#22c55e]/8 p-2 text-[10px] text-[#22c55e]/80">
              {t('formula')}: Total MWh × {EMISSION_FACTOR} tCO₂/MWh = CO₂ avoided
            </div>
      ...
      <div className="impact-popup-anchor relative">
        ...
        {openPopup === 'savings' && (
          <div className="absolute left-1/2 top-full z-[300] mt-2 w-72 -translate-x-1/2 rounded-xl border border-line bg-surface-bg p-4 shadow-2xl animate-fade-in">
            <div className="absolute -top-2 left-1/2 -translate-x-1/2 h-0 w-0 border-l-8 border-r-8 border-b-8 border-transparent border-b-[var(--surface-bg)]" />
            <h4 className="mb-3 text-sm font-semibold text-[#22c55e]">₹ {t('costSavingsBreakdown')}</h4>
            <div className="flex flex-col gap-2 text-xs">
              <div className="flex justify-between border-b border-line pb-1.5">
                <span className="text-muted-text">{t('totalForecastedGen')}</span>
                <span className="font-semibold text-main-text">{totalMwh.toLocaleString('en-IN')} MWh</span>
              </div>
              <div className="flex justify-between border-b border-line pb-1.5">
                <span className="text-muted-text">{t('accuracyImprovement')}</span>
                <span className="font-semibold text-main-text">{(ACCURACY_IMPROVEMENT * 100).toFixed(1)}%</span>
              </div>
              <div className="flex justify-between border-b border-line pb-1.5">
                <span className="text-muted-text">{t('reserveCostRate')}</span>
                <span className="font-semibold text-main-text">₹{RESERVE_COST.toLocaleString('en-IN')}/MWh</span>
              </div>
              <div className="flex justify-between border-b border-line pb-1.5">
                <span className="text-muted-text">{t('spinningReserveSaved')}</span>
                <span className="font-bold text-[#22c55e]">₹{(reserveSaved / 100_000).toFixed(1)} {lang === 'kn' ? 'ಲಕ್ಷ' : 'Lakh'}</span>
              </div>
              <div className="flex justify-between pb-0.5">
                <span className="text-muted-text">{t('annualProjection')}</span>
                <span className="font-semibold text-main-text">₹{((reserveSaved * 365) / 10_000_000).toFixed(1)} {lang === 'kn' ? 'ಕೋಟಿ' : 'Cr'}/yr</span>
              </div>
            </div>
            <div className="mt-3 rounded-md bg-[#22c55e]/8 p-2 text-[10px] text-[#22c55e]/80">
              {t('formula')}: MWh × {(ACCURACY_IMPROVEMENT * 100).toFixed(1)}% × ₹{RESERVE_COST.toLocaleString('en-IN')}/MWh
            </div>
            <div className="mt-1.5 text-[10px] text-faint-text italic text-center">
              {t('basedOnStage2')}
            </div>
